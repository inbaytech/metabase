...
describe("parameters", () => {
  ...
  describe("dashboards", () => {
    ...
    it("should allow creating a public/embedded Dashboard with parameters", async () => {
      ...
      publicDashUrl = Urls.publicDashboard(publicDash.uuid);
    ...
    describe("as an anonymous user", () => {
      ...
      async function runSharedDashboardTests(store, dashUrl) {
        console.log("dashUrl:", dashUrl); // NOCOMMIT
        store.pushPath(dashUrl);
        const app = mount(store.getAppContainer());

        // I copied this line from above. I'm sure it's doing something important.
        await store.waitForActions([ADD_PARAM_VALUES]);
        ...
        await waitForRequestToComplete("GET", "/api/public/dashboard/.*/card/.*");
        ...
        // check that initial value of People Count MBQL Card is 1000 (?)
        /* expect(
         *   app
         *     .update()
         *     .find(Scalar)
        ...
        // now set the SQL param

        // check that value of SQL Card is updated

        // now set our MBQL param
